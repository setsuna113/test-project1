...
from speechbrain.expand.DNS_Challenge.audiolib import wav_read, wav_write
...
def tf_seperation_kernel(num_layer, mask_size, x, stateful=False, numUnits=128, dropout=0.0):
    ...
    for idx in range(num_layer):
        x = layers.LSTM(numUnits, return_sequences=True, stateful=stateful)(x)
...
class InstantLayerNormalization(layers.Layer):
    '''
...
def build_tf_DTLN_model():
    ...
    # calculate STFT
    mag, angle = layers.Lambda(tf_stftLayer)(time_dat)
    mag_norm = mag
    ...
    estimated_mag = layers.Multiply()([mag, mask_1])
    # transform frames back to time domain
    estimated_frames_1 = layers.Lambda(tf_ifftLayer)([estimated_mag, angle])
    ...
    encoded_frames = layers.Conv1D(encoder_size, 1, strides=1, use_bias=False)(estimated_frames_1)
    ...
    estimated = layers.Multiply()([encoded_frames, mask_2])
    # # decode the frames back to time domain
    decoded_frame = layers.Conv1D(blockLen, 1, padding='causal', use_bias=False)(estimated)
    # create waveform with overlap and add procedure
    estimated_sig = layers.Lambda(tf_overlapAddLayer)(decoded_frame)
